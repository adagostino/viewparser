...
function(
  ...
  ViewParser.prototype.createDomObject = function(template, scopeTreeObj, parentDomTree, detached) {
    ...
    scopeTreeObj = this.getScopeTreeFromScope(scopeTreeObj) || scopeTreeObj;
    scopeTreeObj = scopeTreeObj.__className !== 'scopeTree' ? scopeTree.append(scopeTreeObj) : scopeTreeObj;
    ...
    if (parentDomTree && parentDomTree.detachedParentDomTree) {
      ...
      parentDomTree = parentDomTree && parentDomTree.append ? parentDomTree : domTree;
    }
    var childDomTree;
    if (addToDomTree) {
  ...
  ViewParser.prototype.getDomTreeFromElement = function(el) {
    return domTree.getTreeFromElement(el);
  };

  ViewParser.prototype.getScopeTreeFromScope = function($scope) {
    return scopeTree.getTreeFromScope($scope);
  };

  ViewParser.prototype.removeElement = function(el) {
    var domTreeObj = this.getDomTreeFromElement(el);
    if (domTreeObj) {
      domTreeObj.removeSelf();
    } else {
      try {
  ...
  ViewParser.prototype.cleanDomTree = function(domTreeObj) {
    domTreeObj = domTreeObj || domTree;
    if (domTreeObj.el && _isOrphaned(domTreeObj.el)) {
      domTreeObj.removeSelf();
    } else {
      for (var i=0; i<domTreeObj.children.length; i++) {
        this.cleanDomTree(domTreeObj.children[i]);
      }
    }
  };
